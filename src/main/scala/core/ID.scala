...
class ID extends Module{
    val io = IO(new Bundle{
        val in = new Bundle{
            ...
            val stall_io= Flipped(new Stall_IO())
            val mem_io  = Flipped(new WB_IO())
            val wb_io   = Flipped(new WB_IO())
        }
        val out = new ID_IO()
    })
    ...
    val stall_flag      = io.in.stall_io.stall_flag
    val pred_miss_flag   = io.in.stall_io.pred_miss_flag
    val mem_rd_wen      = io.in.mem_io.rd_wen
    val mem_rd_addr     = io.in.mem_io.rd_addr
    val mem_rd_data     = io.in.mem_io.rd_data
    val wb_rd_wen       = io.in.wb_io.rd_wen
    val wb_rd_addr      = io.in.wb_io.rd_addr
    val wb_rd_data      = io.in.wb_io.rd_data
